...
class NPHM(MessagePassing):
    def __init__(self,
        ...
        self.deepSDFensemble = EnsembleDeepSDF(ensemble_size=self.n_anchors,
                                               n_symm=self.n_symm,
                                               lat_dim=d_in_geo,
                                               hidden_dim=self.hidden_dim//2 if half_dim_geo else self.hidden_dim,
        ...
        if self.color_branch:
            ...
            if self.color_communication:
                ...
                self.communcation_bottleneck1 = torch.nn.Linear(
                                                            in_features=self.hidden_dim//2 if half_dim_geo else self.hidden_dim,
    ...
    def reset_parameters(self):
        ...
        if self.color_branch:
            for lin in self.aggr_app.layers:
                lin.reset_parameters()
...
def get_id_model(cfg,
    ...
    assert cfg_id['gnn']['hidden_dim_geo'] == cfg_id['gnn']['hidden_dim_app']
    assert cfg_id['gnn']['nlayers_geo'] == cfg_id['gnn']['nlayers_app']
    id_model = NPHM(
