...
from nphm_tum.data.mvs_dataset import MVSdataset, MVSdatasetRendering
from nphm_tum.data.face_dataset import NPHMdataset

# NNs
from nphm_tum.models.canonical_space import get_id_model
from nphm_tum.models.deformations import ZeroDeformation, DeformationNetwork
from nphm_tum.models.neural3dmm import LatentCodes
...
def set_up_datasets(cfg,
    ...
    train_dataset = DataSetClass(shared_dict_train,
                                 mode='train',
                                      ...
                                      overfit_id=None, #overfit_id,
                                      use_patches=use_patches,
                                      npatches_per_batch=cfg['training']['npatches_per_batch'] if use_patches else 0,
                                      debug_run=debug_run,
                                      fetch_uvs=uv_communication,
                                      )
    val_dataset = DataSetClass(shared_dict_val, mode='val',
                                   n_supervision_points_corresp=cfg['training'].get('npoints_corresp', 250),
                                   n_supervision_points_face=cfg['training'].get('npoints_face', 1000),
                                   n_supervision_points_non_face=cfg['training'].get('npoints_non_face', 250),
                                   n_supervision_points_off_surface=cfg['training'].get('npoints_off_surface', 250),
    ...
    if mvs_dataset:
        assert 1 == 2, 'num_symm and mirroring not imple'
        train_dataset_render = MVSdatasetRendering(None,
                                                    mode='train',
                                                    n_supervision_points_corresp=cfg['training']['npoints_corresp'],
                                                    n_supervision_points_face=cfg['training']['npoints_face'],
                                                    n_supervision_points_non_face=cfg['training']['npoints_non_face'],
                                                    n_supervision_points_off_surface=cfg['training'][
                                                        'npoints_off_surface'],
                                                    batch_size=cfg['training']['batch_size'],
                                                    # sigma_near=CFG['training']['sigma_near'], #TODO probably should use that?!
                                                    lm_inds=lm_inds,
        ...
        val_dataset_render = MVSdatasetRendering(None, mode='val',
                                                  n_supervision_points_corresp=cfg['training']['npoints_corresp'],
                                                  n_supervision_points_face=cfg['training']['npoints_face'],
                                                  n_supervision_points_non_face=cfg['training']['npoints_non_face'],
                                                  n_supervision_points_off_surface=cfg['training'][
        ...
        train_dataset_render = None
        val_dataset_render = None

    return train_dataset, val_dataset, train_dataset_render, val_dataset_render
...
def set_up_codes(cfg,
    ...
    if include_app:
        ...
        n_train_app = len(np.unique(np.array(train_dataset.appearance_index))) if train_dataset is not None else 381
        n_val_app = len(np.unique(np.array(val_dataset.appearance_index))) if train_dataset is not None else 10
        if codebook_numbers_train is not None:
...
def set_up_all(cfg,
        ...
        if not skip_dataset:
            train_dataset, val_dataset, train_dataset_render, val_dataset_render = set_up_datasets(cfg,
                                                                                               lm_inds=lm_inds,
                                                                                               **args
                                                                                               )
        else:
        ...
        if not skip_dataset:
            print(f'Train Dataset has {len(train_dataset.subjects)} Subjects and {len(train_dataset.subject_steps)} Expressions')
            print(f'Val Dataset has {len(val_dataset.subjects)} Subjects and {len(val_dataset.subject_steps)} Expressions')

        return {
            'train_dataset': train_dataset,
            'val_dataset': val_dataset,
            'train_dataset_render': train_dataset_render,
            'val_dataset_render': val_dataset_render,
