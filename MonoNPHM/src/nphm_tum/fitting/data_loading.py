...
from nphm_tum.utils.renderer import m3dLookAt, back_project
from nphm_tum.utils.tranformations import invert_similarity_transformation
...
def prepare_data(timestep=None,
    ...
    #mica_pose = np.load(f'{env_paths.MICA_TRACKER_OUTPUT_PATH}/s{seq_name[6:]}/checkpoint/{max(timestep - 1, 0):05d}_cam_params_opencv.npz')
    new_mica_pose = np.load(f'{env_paths.DATA_TRACKING}/{seq_name}/metrical_tracker/{seq_name}/checkpoint/{max(timestep - 1, 0):05d}_cam_params_opencv.npz')
    mica_pose = new_mica_pose

    if intrinsics_provided:
        # load intrinsics parameters
        calibration_folder = f'{env_paths.DATA_TRACKING}/{seq_name}/camera/c00_color_intrinsic.txt'
        ...
        K = mica_pose['K'][0, :, :]
        # load intrinsics parameters
        #calibration_folder = f'{env_paths.KINECT_raw}/{seq_name}/camera/c00_color_intrinsic.txt'
        #with open(calibration_folder, 'r') as fp:
        #    print("Loading intrinsics from JSON")
        #    calibration = fp.readline().split()
        #    fx = float(calibration[2])
        ...
        #    cy = float(calibration[1])
        ## construct intrinsics matrix
        #K_og = np.array([[fx, 0, cx], [0, fy, cy], [0, 0, 1]])
    ...
    rgb = Image.open(f'{env_paths.DATA_TRACKING}/{seq_name}/source/{timestep:05d}.png')
    normal_map =  Image.open(f'{env_paths.DATA_TRACKING}/{seq_name}/normals/{timestep:05d}.png')
    ...
    normal_map = normal_map[top_start:top_start + box_height, left_start:left_start + box_width, :]
    ...
    normal_map = Image.fromarray(normal_map).resize((rend_size[1], rend_size[0]))
    ...
    foreground_mask = rgb_loss_mask | (facer_mask_hq == 1) | (facer_mask_hq == 14) # include neck and hair
    facer_mask_hq[~foreground_mask] = 0
    ...
    if intrinsics_provided:
        fa_lms = np.load(
            f'{env_paths.DATA_TRACKING}/{seq_name}/kpt/{timestep:05d}.npy') / 512 * 1080  # downsample_factor

        #im_tmp = rgb_og #cv2.imread('/mnt/hdd/debug_prep/simon_510_s4/images/00001.png')
        fa_lms[:, 0] = fa_lms[:, 0] + 420
    ...
    normal_map = ((np.array(normal_map) / 255) - 0.5) * 2

    relevant_normals = (facer_mask >= 8) & (facer_mask <= 13)

    relevant_normals = ndimage.binary_dilation(relevant_normals, iterations=5)

    normal_map[~relevant_normals] = 0
    ...
    normal_map = torch.from_numpy(normal_map).reshape(-1, 3)
    ...
    data['normal_map'].append(normal_map)
