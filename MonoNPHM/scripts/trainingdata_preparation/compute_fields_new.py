...
def run_subject(s, expressions=None):
    ...
    for e in expressions:
        ...
        if osp.exists(manager.get_train_path_identity_off_surface(s, e, rnd_file=NUM_SPLITS-1)) and not VISUALIZE:
            print('SKIPPING:', s, e)
            continue
        else:
            print('Running: ', s, e)
            print(manager.get_train_path_identity_off_surface(s, e, rnd_file=NUM_SPLITS-1))
        try:
            print(s, e)
            N_SAMPS = 25000000
            N_SAMPS_OFF = 1000000
                ...
                if not OFF_SURFACE_ONLY:
                    ...
                    for i, chunk_non_face in enumerate(chunks_non_face):
                        ...
                        for i, chunk_outer in enumerate(chunks_outer):
                            np.save(manager.get_train_path_identity_deformation_region(s, e, rnd_file=i), chunk_outer)
                for i, chunk_off in enumerate(chunks_off):
                    np.save(manager.get_train_path_identity_off_surface(s, e, rnd_file=i), chunk_off)
